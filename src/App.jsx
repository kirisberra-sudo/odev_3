...
const initialState = {
  ...
  episodes: [],
  detailedShow: null,
  watchlist: JSON.parse(localStorage.getItem('movieClubWatchlist')) || [],
  isLoading: true,
  isError: false,
  query: localStorage.getItem('movieClubQuery') || "Avrupa Yakası",
  filters: {
    genre: "",
    language: "",
    rating: 0,
...
function Footer({ name }) {
  return (
    <footer className="app-footer">
      <p className="footer-text">&copy; 2025 {"Berra Kırış"}. Tüm hakları saklıdır.</p>
    </footer>
  );
}
  ...
  return (
    <>
      <header className="app-header">
        <h1 className="app-title">Kampüs Film Kulübü</h1>
      </header>
...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem('movieClubQuery', state.query);
  }, [state.query]);

  useEffect(() => {
    localStorage.setItem('movieClubWatchlist', JSON.stringify(state.watchlist));
  }, [state.watchlist]);
